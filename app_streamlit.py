...
if st.session_state.df_original is not None:
    # Top Level Tabs
    top_tab2, top_tab1 = st.tabs(["🛠️ Data Engine", "🤖 PrometheusAI Agent"])
    
    # --- Tab 1: Agent ---
    with top_tab1:
        if "df_cleaned" in st.session_state and st.session_state.df_cleaned is not None:
             agent.ui.render_agent_tab(st.session_state.df_cleaned)
        elif "df_original" in st.session_state:
             agent.ui.render_agent_tab(st.session_state.df_original)

    # --- Tab 2: Data Engine (Original Tabs) ---
    with top_tab2:
